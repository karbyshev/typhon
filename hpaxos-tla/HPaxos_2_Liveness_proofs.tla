...
\* TODO remove if ValueExistence moved to another file
...
\* \A alpha
\* \E val \in Value : \E x \in Message : M \in SUBSET Tran(x)
\* x is a proposal of val in bal and B(x) >= B(m), for all m \in M
\* /\ \A m \in Latest({ mm \in M : D(alpha, x, mm) }) : V(m, val)
...
LEMMA ValueExistence ==
    ...
    PROVE  \E bal \in Ballot : \E val \in Value : \E x \in Message :
            ...
            /\ B(x, bal)
            /\ val = BVal[bal]
            /\ \A m \in M : \A mbal \in Ballot : B(m, mbal) => mbal =< bal
            /\ M \in SUBSET Tran(x)
            /\ \A m \in Latest({ mm \in M : D(alpha, x, mm) }) : V(m, val)
PROOF
...
<1> y \in Message /\ Proposal(y)
    BY OneA_Message_bis DEF Proposal, OneA
<1> y.prev = NoMessage
    OBVIOUS
<1> PICK B0 \in Ballot :
            \A m \in M : \A b \in Ballot : B(m, b) => b =< B0
    BY BallotOfSet
<1> DEFINE Ly == Latest({ mm \in M : D(alpha, y, mm) })
<1>0. CASE Ly = {}
  <2> PICK val \in Value : TRUE
      BY ValueNonEmpty
  <2> PICK bal \in Ballot : B0 =< bal /\ val = BVal[bal]
      BY BValValueAssumption
  <2>1. \A m \in M : \A mbal \in Ballot : B(m, mbal) => mbal =< bal
      BY DEF Ballot
  ...
  <2>2. Latest({ mm \in M : D(alpha, x, mm) }) = Latest({ mm \in M : D(alpha, y, mm) })
    <3> HIDE DEF x, y
    <3> QED BY XXX DEF D
  <2>3. Latest({ mm \in M : D(alpha, x, mm) }) = {}
        BY <1>0, <2>2
  <2>4. M \in SUBSET Tran(x)
        BY Tran_eq, Tran_refl
  <2> HIDE DEF x
  <2> WITNESS bal \in Ballot, val \in Value, x \in Message
  <2> QED BY <2>1, <2>bal, <2>3, <2>4
<1>1. CASE Ly # {}
  <2> PICK m \in Latest({ mm \in M : D(alpha, y, mm) }) : TRUE
      BY <1>1
  <2> QED
<1> QED BY <1>0, <1>1
...
\* Last modified Wed Jun 25 11:52:05 CEST 2025 by karbyshev
