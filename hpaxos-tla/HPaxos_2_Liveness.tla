        ...
        /\ LET p == [ type |-> "1a", bal |-> bal, prev |-> NoMessage, refs |-> M ] IN
            ...
            /\ (\A x \in Message :
                \A xbal \in Ballot :
                    ...
                    x = p)
\*        /\   \* val ... related to alpha
\*            (\A acc \in SafeAcceptor :
\*            <> (m2a \in known_msgs[acc])
\*            )
        => <> ChosenIn(alpha, bal, val)
    \* We cannot force that every safe acceptor has learned about m2a because fake acceptors can send 2a to some safe acceptors, not all.
